...
class TestICATGetWithFilters:
    ...
    def test_valid_no_results_get_with_filters(
        ...
        test_response = test_client.get(
            '/datagateway_api/investigations?where={"title": {"eq": "This filter should cause a 404 fortesting purposes..."}}',
            headers=valid_icat_credentials_header,
        )
    ...
    def test_valid_get_with_filters_distinct(
        ...
        test_response = test_client.get(
            '/datagateway_api/investigations?where={"title": {"like": "Test data for Python ICAT on '
