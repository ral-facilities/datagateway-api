...
class InvestigationInstruments(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(INVESTIGATIONINSTRUMENT, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(INVESTIGATIONINSTRUMENT, request.json))), 200
...
class InvestigationInstrumentsCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(INVESTIGATIONINSTRUMENT, filters), 200
...
class InvestigationInstrumentsFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(INVESTIGATIONINSTRUMENT, filters), 200
