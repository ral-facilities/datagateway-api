...
class Jobs(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(JOB, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(JOB, request.json))), 200
...
class JobsCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(JOB, filters), 200
...
class JobsFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(JOB, filters), 200
