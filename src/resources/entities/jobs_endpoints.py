...
class Jobs(Resource):
    ...
    @queries_records
    def post(self):
        create_row_from_json(JOB, request.json)
        return get_row_by_id(JOB, request.json["id"].to_dict()), 200
...
class JobsWithID(Resource):
    ...
    @queries_records
    def get(self, id):
        return get_row_by_id(JOB, id).to_dict(), 200

    @requires_session_id
    @queries_records
    def delete(self, id):
        delete_row_by_id(JOB, id)
        return "", 204

    @requires_session_id
    @queries_records
    def patch(self, id):
        update_row_from_id(JOB, id, str(request.json))
        return get_row_by_id(JOB, id).to_dict(), 200
