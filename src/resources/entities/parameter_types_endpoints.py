...
class ParameterTypes(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(PARAMETERTYPE, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(PARAMETERTYPE, request.json))), 200
...
class ParameterTypesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(PARAMETERTYPE, filters), 200
...
class ParameterTypesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(PARAMETERTYPE, filters), 200
