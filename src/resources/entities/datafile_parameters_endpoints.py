...
class DatafileParameters(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(DATAFILEPARAMETER, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(DATAFILEPARAMETER, request.json))), 200
