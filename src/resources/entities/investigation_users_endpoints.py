...
class InvestigationUsers(Resource):
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(INVESTIGATIONUSER, request.json))), 200
...
class InvestigationUsersCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(INVESTIGATIONUSER, filters), 200
...
class InvestigationUsersFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(INVESTIGATIONUSER, filters), 200
