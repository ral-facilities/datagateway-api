...
class InvestigationTypes(Resource):
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(INVESTIGATIONTYPE, request.json))), 200
...
class InvestigationTypesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(INVESTIGATIONTYPE, filters), 200
...
class InvestigationTypesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(INVESTIGATIONTYPE, filters), 200
