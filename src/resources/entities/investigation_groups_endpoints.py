...
class InvestigationGroups(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(INVESTIGATIONGROUP, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(INVESTIGATIONGROUP, request.json))), 200
...
class InvestigationGroupsWithID(Resource):
    ...
    @queries_records
    def patch(self, id):
        EntityManager.update_row_from_id(INVESTIGATIONGROUP, id, str(request.json))
        return EntityManager.get_row_by_id(INVESTIGATIONGROUP, id).to_dict(), 200
...
class InvestigationGroupsCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(INVESTIGATIONGROUP, filters), 200
...
class InvestigationGroupsFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(INVESTIGATIONGROUP, filters), 200
