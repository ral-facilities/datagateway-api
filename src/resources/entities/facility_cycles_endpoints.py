...
class FacilityCycles(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(FACILITYCYCLE, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(FACILITYCYCLE, request.json))), 200
...
class FacilityCyclesWithID(Resource):
    ...
    @queries_records
    def patch(self, id):
        EntityManager.update_row_from_id(FACILITYCYCLE, id, str(request.json))
        return EntityManager.get_row_by_id(FACILITYCYCLE, id).to_dict(), 200
...
class FacilityCyclesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(FACILITYCYCLE, filters), 200
...
class FacilityCyclesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(FACILITYCYCLE, filters), 200
