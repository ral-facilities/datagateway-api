...
class InstrumentScientists(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(INSTRUMENTSCIENTIST, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(INSTRUMENTSCIENTIST, request.json))), 200
...
class InstrumentScientistsCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(INSTRUMENTSCIENTIST, filters), 200
...
class InstrumentScientistsFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(INSTRUMENTSCIENTIST, filters), 200
