...
class DataCollectionDatafiles(Resource):
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(DATACOLLECTIONDATAFILE, request.json))), 200
...
class DataCollectionDatafilesWithID(Resource):
    ...
    @queries_records
    def delete(self, id):
        EntityManager.delete_row_by_id(DATACOLLECTIONDATAFILE, id)
        return "", 204

    @requires_session_id
    @queries_records
    def patch(self, id):
        EntityManager.update_row_from_id(DATACOLLECTIONDATAFILE, id, str(request.json))
        return EntityManager.get_row_by_id(DATACOLLECTIONDATAFILE, id).to_dict(), 200
...
class DataCollectionDatafilesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(DATACOLLECTIONDATAFILE, filters), 200
...
class DataCollectionDatafilesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(DATACOLLECTIONDATAFILE, filters), 200
