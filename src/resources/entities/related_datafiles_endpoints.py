...
class RelatedDatafiles(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(RELATEDDATAFILE, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(RELATEDDATAFILE, request.json))), 200
...
class RelatedDatafilesWithID(Resource):
    ...
    @queries_records
    def delete(self, id):
        EntityManager.delete_row_by_id(RELATEDDATAFILE, id)
        return "", 204

    @requires_session_id
    @queries_records
    def patch(self, id):
        EntityManager.update_row_from_id(RELATEDDATAFILE, id, str(request.json))
        return EntityManager.get_row_by_id(RELATEDDATAFILE, id).to_dict(), 200
...
class RelatedDatafilesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(RELATEDDATAFILE, filters), 200
...
class RelatedDatafilesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(RELATEDDATAFILE, filters), 200
