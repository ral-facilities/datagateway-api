...
class PermissibleStringValues(Resource):
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(PERMISSIBLESTRINGVALUE, request.json))), 200
...
class PermissibleStringValuesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(PERMISSIBLESTRINGVALUE, filters), 200
...
class PermissibleStringValuesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(PERMISSIBLESTRINGVALUE, filters), 200
