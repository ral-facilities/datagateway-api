...
class Facilities(Resource):
    ...
    @queries_records
    def post(self):
        return create_rows_from_json(request.json, FACILITY), 200
...
class FacilitiesWithID(Resource):
    ...
    @queries_records
    def get(self, id):
        return get_row_by_id(FACILITY, id).to_dict(), 200

    @requires_session_id
    @queries_records
    def delete(self, id):
        delete_row_by_id(FACILITY, id)
        return "", 204

    @requires_session_id
    @queries_records
    def patch(self, id):
        update_row_from_id(FACILITY, id, str(request.json))
        return get_row_by_id(FACILITY, id).to_dict(), 200
