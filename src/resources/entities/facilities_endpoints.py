...
class Facilities(Resource):
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(FACILITY, request.json))), 200
...
class FacilitiesCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(FACILITY, filters), 200
...
class FacilitiesFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return EntityManager.get_first_filtered_row(FACILITY, filters), 200
