...
class Publications(Resource):
    ...
    @queries_records
    def get(self):
        return EntityManager.get_rows_by_filter(PUBLICATION, get_filters_from_query_string()), 200
    ...
    @queries_records
    def patch(self):
        return list(map(lambda x: x.to_dict(), EntityManager.patch_entities(PUBLICATION, request.json))), 200
...
class PublicationsCount(Resource):
    ...
    def get(self):
        ...
        return EntityManager.get_filtered_row_count(PUBLICATION, filters), 200
...
class PublicationsFindOne(Resource):
    ...
    def get(self):
        filters = get_filters_from_query_string()
        return (PUBLICATION, filters), 200
