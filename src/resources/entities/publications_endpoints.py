...
class Publications(Resource):
    ...
    @queries_records
    def post(self):
        create_row_from_json(PUBLICATION, request.json)
        return get_row_by_id(PUBLICATION, request.json["id"].to_dict()), 200
...
class PublicationsWithID(Resource):
    ...
    @queries_records
    def get(self, id):
        return get_row_by_id(PUBLICATION, id).to_dict(), 200

    @requires_session_id
    @queries_records
    def delete(self, id):
        delete_row_by_id(PUBLICATION, id)
        return "", 204

    @requires_session_id
    @queries_records
    def patch(self, id):
        update_row_from_id(PUBLICATION, id, str(request.json))
        return get_row_by_id(PUBLICATION, id).to_dict(), 200
